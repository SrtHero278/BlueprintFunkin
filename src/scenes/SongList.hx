...
class SongList extends BaseMenu {
	...
	public static function getSongs(folders:Array<String>) {
		for (song in folders) {
			...
			final path = Paths.songFile("", song);
			final metaPathNoExt = path + "songMeta";
			final multiDiffFormat = GameSong.findMultiDiff(path + "diffs", metaPathNoExt);
			if (multiDiffFormat != null) {
				final metaPath:Null<String> = (multiDiffFormat.hasMetaFile != FALSE) ? (metaPathNoExt + "." + multiDiffFormat.metaFileExtension) : null;
				var inst:DynamicFormat = cast Type.createInstance(multiDiffFormat.handler, []);
				inst.fromFile(path + "diffs." + multiDiffFormat.extension, metaPath);
			...
			for (file in FileSystem.readDirectory(diffPath)) {
				...
				final format = GameSong.trySingleDiff(filePath, metaPathNoExt);
